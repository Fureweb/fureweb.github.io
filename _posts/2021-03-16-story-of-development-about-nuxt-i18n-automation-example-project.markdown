...
* 위 문서는 ko 필드에만 한글 텍스트를 넣었고, en, ja 필드 값은 전부 [GOOGLETRANSLATE 함수](https://goree.kr/imaDrw)를 통해 구글이 번역해준 결과를 사용하도록 해 두었습니다. 실제 번역된 텍스트가 나오기 전 까지 임시로 사용할 수 있을 것이며, 이 부분은 최종적으로 검수를 마친 텍스트로 대체되어야 할 것입니다.
