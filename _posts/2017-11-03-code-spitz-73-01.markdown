...
Lint Time에서 Compile Time을 거치지 않고 바로 Run Time으로 넘어갑니다. 헌대의 자바스크립트는 작성된 코드를 Auto Compile 하는데요. 다음과 같은 코드가 있을 때, 자바스크립트는 어떻게 처리할까요?
```js
...
console.log('aa');
```
testFunction을 선언했지만, 호출하지 않았기 때문에 오류가 발생하지 않고, 콘솔에는 aa가 출력되어 있을 것입니다. 분명 testFunction을 선언하면서 내부에 정의되지 않은 변수를 사용하고 있음에도 아무런 오류가 검출되거나, 코드 전체가 깨지는 일이 일어나지 않습니다. 이는 자바스크립트가 부분적으로 컴파일하는 전략[(JIT compilation)](https://en.wikipedia.org/wiki/Just-in-time_compilation){:target="_blank"}을 사용하고 있음을 확인할 수 있는 부분입니다.

하지만 이렇게 컴파일 되기 전의 중간 과정이 하나 더 있어서, 아래와 같은 코드는 애초에 바이트코드로 만들어주지도 않고 파싱 자체를 멈추며 오류를 뿜어냅니다.
```js
const testFunction =_=> {
    aaaaa bbbbb ccccc; // 여기는 퇴짜를 맞습니다.
}

console.log('aa');
```

즉, 자바스크립트에는 사실상 오류 검출의 시점으로 보았을 때 Compile Time의 완충재가 없기 때문에 TypeScript같은 Javascript의 Superset을 이용하기도 합니다. 또는...
